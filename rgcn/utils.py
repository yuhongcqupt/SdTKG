...
def r2_so(triplets):
    ...
    uniq_r = np.unique(rel)
    # generate r2e
    r_to_s = defaultdict(set)
    r_to_o = defaultdict(set)
    for j, (src, rel, dst) in enumerate(triplets):
        r_to_s[rel].add(src)
        r_to_o[rel].add(dst)
    r_len = []
    ...
    idx = 0
    for r in uniq_r:
        r_len.append((idx, idx + len(r_to_s[r])))
        s_idx.extend(list(r_to_s[r]))
        o_idx.extend(list(r_to_o[r]))
        idx += len(r_to_s[r])
    return uniq_r, r_len, s_idx, o_idx
...
def build_sub_hete_graph(num_nodes,  triples, path_triples, h1, use_cuda, gpu):

    triples = triples[:, :3]
    ori_src, ori_rel, ori_dst = triples.transpose()
    groups = {}
    g_list = {}

    if len(path_triples)!=0:
        #print(path_triples)
        triple = np.append(path_triples[:, :3],triples,axis=0)
        triple = np.append(triple,h1[:,:3],axis=0)
    else:
        triple = triples
        triple = np.append(triple, h1[:, :3], axis=0)
    #print(triples.shape)
    ...
    for key,group in groups.items():
        ...
        for s in src:
            if s in ori_src:
                g.add_edges([s] * len(src), src)
                point.append(s)
            elif s in ori_dst:
                g.add_edges([s] * len(dst), dst)
                point.append(s)

        g.point_set=np.unique(point)
        g.dst_set = dst
        g.src_set = src
        if use_cuda:
            g.to(gpu)
...
def build_pre_graph(num_nodes, num_rels, triples, all_path_g, use_cuda,  gpu, path_dict):
    triple = triples[:, :3]
    #src, rel, dst = triples.transpose()
    ...
    #print(len(src))
    #print(all_path_g.out_edges(torch.tensor([1]),form='uv'))

    edge = [torch.tensor([all_path_g.edata['type'][j.item()] for j in all_path_g.out_edges(torch.tensor(i), form='eid')]) for i in src]
    o = [all_path_g.out_edges(torch.tensor(i),form='uv')[1] for i in src]
    g.edata.update({'path': torch.nn.utils.rnn.pad_sequence(edge, batch_first=True, padding_value=-1),
                    'o':torch.nn.utils.rnn.pad_sequence(o, batch_first=True, padding_value=-1)})
    target_mask=[]
    num=0
    for i in range(len(o)):
        l=[]
        for j in o[i]:
            ans = np.where((triple[0] == src[i]) & (triple[1] == rel[i]) & (triple[2] == j.item()))
            if len(ans[0])>0 and j.item() not in l:
                #print(triple[ans])
                l.append(j.item())
                #print(ans)

                #print([src[i],rel[i],j.item()])
        num = num + len(l)
        target_mask.append(l)


    if use_cuda:
        g.to(gpu)
    return g,target_mask

def build_path_graph(num_nodes,  triples,  use_cuda, gpu):
    src = triples[:,0]
    rel = triples[:,1]
    dst = triples[:,2]
...
def build_all_time_graph(num_nodes,  triples,  use_cuda, gpu):
    #print(triples.shape)
    #print(triples)
    g = dgl.DGLGraph()
    g.add_nodes(num_nodes)
    ...
    for i in range(len(triples)):
        ...
        rel=triples[i][:,1]
        dst=triples[i][:,2]
        #src, rel, dst = triples[i]
        rel=torch.tensor(rel)
        #g.add_edges(src,dst,{'time':torch.tensor([i]*len(dst)),'type': rel})
        g.add_edges(src, dst, { 'type': rel})
    node_id = torch.arange(0, num_nodes, dtype=torch.long).view(-1, 1)
    g.ndata.update({'id': node_id})
    if use_cuda:
        g.to(gpu)
...
def path_r2e(triplets, num_rels):
    src, rel, dst = triplets.transpose()
    # get all relations
    uniq_r = np.unique(rel)
    uniq_r = np.concatenate((uniq_r, uniq_r + num_rels))
    # generate r2e
    r_to_s = defaultdict(list)
    r_to_o = defaultdict(list)

    for j, (src, rel, dst) in enumerate(triplets):
        r_to_s[rel].append(src)
        r_to_s[rel+num_rels].append(dst)
        r_to_o[rel].append(dst)
        r_to_o[rel+num_rels].append(src)
    r_len = []
    s_idx = []
    o_idx = []
    idx = 0
    for r in uniq_r:
        ...
        o_idx.extend(list(r_to_o[r]))
        idx += len(r_to_s[r])
    return uniq_r, r_len, r_to_s, r_to_o
...
def load_data(dataset, bfs_level=3, relabel=False):
    if dataset in ['aifb', 'mutag', 'bgs', 'am']:
        ...
        return knwlgrh.load_link(dataset)
                     ...
                     "WIKI", 'icews14', 'icews18s', 'icews0515s', 'icews14s-tirgn']:
        return knwlgrh.load_from_local("../data", dataset)
    elif dataset in ['icews14s-tirgn']:
        return knwlgrh.load_from_local_id("../data", dataset)
    else:
