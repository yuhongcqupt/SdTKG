...
class GATLayer(nn.Module):
    def __init__(self, in_feat, out_feat, num_heads=1):
        ...
        self.gatconv= GATConv(self.in_feat, self.out_feat, num_heads=self.num_heads, allow_zero_in_degree=True)
    ...
    def forward(self,g,feat):
        ...
        node_repr=self.gatconv(g,feat)

        return node_repr

    def msg_func(self, edges):
        edge_type = edges.data['type']
        src = edges.src['h'].view(-1, self.out_feat)
        dst = edges.dst['h'].view(-1, self.out_feat)

        batch_size = src.shape[0]
        stacked_inputs=batch_size
        msg = stacked_inputs.view(batch_size, -1)
        return {'msg': msg}
